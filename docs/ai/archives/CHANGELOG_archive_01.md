# Changelog — NoSeumCode (Archive 01 : Sessions 1 à 20)
...
- **Résolution de la dépendance circulaire de beans Spring** : Extraction des beans `JwtEncoder`, `JwtDecoder` et `PasswordEncoder` dans une classe de configuration dédiée `JwtConfig`, brisant le cycle entre `SecurityConfig`, `OAuth2AuthenticationSuccessHandler` et `JwtService`.
...
**Réalisé** :
- **Composant Pop-up Universel `auth-popover` (`popovers-shared.html`)** :
...
- **Header Global (`header.html` & `header.js`)** :
...
- **Styles CSS (`popover.css`, `homepage.css`, `homepage.min.css`)** :
...
**Réalisé** :
- **Backend (`AuthService.java`)** : Fixation stricte du rôle à `Role.STUDENT` lors de toute inscription publique.
- **Pop-up Inscription (`popovers-shared.html` & `header.js`)** : Retrait du sélecteur de rôle dans le formulaire d'inscription.
...
- **Logique Dashboard (`dashboard.js`)** : Affichage automatique et exclusif de la vue dédiée (`#view-student`, `#view-teacher`, ou `#view-admin`) selon le rôle réel de l'utilisateur.
- **Styles (`dashboard.css`)** : Harmonisation complète avec la palette NoSeumCode (fond clair `#f5f7fa`, cartes blanches `#ffffff`, ombres subtiles, typographies Poppins et Bangers).
...
**Réalisé** :
- **Seeder Spring Boot (`AdminSeeder.java`)** : Composant `CommandLineRunner` exécuté à chaque démarrage du backend pour vérifier l'existence de `admin@codebangers.fr`, chiffrer le mot de passe `778195` avec `BCryptPasswordEncoder` et attribuer le rôle `ADMIN`.
- **Migration Flyway V006 (`V006__seed_admin_user.sql`)** : Script SQL de seed automatique insérant/mettant à jour le compte administrateur avec mot de passe haché BCrypt et rôle `ADMIN`.
...
**Réalisé** :
- **Contrôle d'accès & API REST Backend (`UserController.java` & `UserService.java`)** :
...
- **Interface Administrateur (`dashboard.html` & `dashboard.js`)** :
...
**Réalisé** :
- **Backend (`AdminSeeder.java`, `application.properties`, `.env`)** :
  ...
  - Création du template sécurisé `backend/.env.example`.
  - Renforcement de `.gitignore` pour protéger `.env`, `.env.*` et `*.env`.
- **Frontend (`auth.ts`, `.env`, `.env.example`)** :
  - Suppression de la chaîne de connexion PostgreSQL en dur dans `auth.ts` au profit de `process.env.DATABASE_URL`.
  - Harmonisation du fichier `frontend/.env` et création du template `frontend/.env.example`.
...
**Réalisé** :
- **Frontend (`auth.ts`, `auth-client.ts`, `.env`, `.env.example`)** :
...
- **Backend (`application.properties`, `.env`, `.env.example`)** :
...
**Réalisé** :
- **Backend (`UserController.java`, `UserService.java`, `UserResponse.java`, `CourseResponse.java`, `CourseController.java`)** :
...
**Réalisé** :
- **Backend (`PaymentService.java`, `PaymentController.java`, `PaymentStatusUpdateRequest.java`, `SecurityConfig.java`)** :
...
- **Frontend (`dashboard.js`, `dashboard.css`)** :
...
**Réalisé** :
- **Frontend (`popovers-shared.html`, `header.js`, `dashboard.js`)** :
...
**Réalisé** :
- **Backend (`V007__seed_courses_chapters_content.sql`, `CourseController.java`, `ChapterController.java`)** :
...
- **Frontend (`article.html`, `article.js`, `article.css`)** :
...
**Réalisé** :
- **Frontend Articles (`article.html`, `article.js`)** :
...
- **Frontend Formations (`cours.html`, `course.html`, `cours.js`, `cours.css`)** :
...
**Réalisé** :
- **Backend (`V007`, `V008`, `CourseController.java`, `ChapterController.java`, `EnrollmentController.java`)** :
...
- **Frontend (`cours.html`, `cours.js`, `cours.css`, `header.html`)** :
